    ...
    <div>
      <.header>
        ...
        <:actions>
          ...
          <.button type="button" phx-click="introspect" class="sui-primary ml-4">
            Refresh Schema
          </.button>
        </:actions>
      </.header>
      ...
      <.main_content>
        <%= if @schema do %>
          ...
          <div class="flex gap-2 mb-4">
            <button type="button" phx-click="set_tab" phx-value-tab="queries"
              class={"px-4 py-2 text-sm rounded-lg transition-colors #{if @active_tab == "queries", do: "bg-indigo-600 text-white", else: "bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300"}"}>
              Queries (<%= length(@schema["queries"] || []) %>)
            </button>
            <button type="button" phx-click="set_tab" phx-value-tab="mutations"
              class={"px-4 py-2 text-sm rounded-lg transition-colors #{if @active_tab == "mutations", do: "bg-indigo-600 text-white", else: "bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300"}"}>
              Mutations (<%= length(@schema["mutations"] || []) %>)
            </button>
            <button type="button" phx-click="set_tab" phx-value-tab="types"
              class={"px-4 py-2 text-sm rounded-lg transition-colors #{if @active_tab == "types", do: "bg-indigo-600 text-white", else: "bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300"}"}>
        ...
        <% else %>
          <div class="text-center py-12">
            <p class="text-gray-500 dark:text-gray-400 mb-4">No schema loaded for this endpoint.</p>
            <.button type="button" phx-click="introspect" class="sui-primary">
              Introspect Schema
            </.button>
          </div>
        <% end %>
  ...
  defp operation_card(assigns) do
    ...
    <div class="p-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg">
      <div class="flex items-start justify-between">
        <div>
          <h4 class="text-sm font-semibold text-gray-900 dark:text-gray-100 font-mono"><%= @operation["name"] %></h4>
          <p :if={@operation["description"]} class="mt-1 text-xs text-gray-500 dark:text-gray-400"><%= @operation["description"] %></p>
        </div>
        <span class="text-xs font-mono text-indigo-600 dark:text-indigo-400"><%= type_display(@operation["type"]) %></span>
      </div>
          ...
          <div class="mt-1 space-y-1">
            <%= for arg <- @operation["args"] do %>
              <div class="flex items-center gap-2 text-xs">
                <span class="font-mono text-gray-700 dark:text-gray-300"><%= arg["name"] %></span>
                <span class="font-mono text-indigo-500 dark:text-indigo-400"><%= type_display(arg["type"]) %></span>
  ...
  defp type_card(assigns) do
    ...
    <div class="p-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg">
      <div class="flex items-center gap-2">
        <span class="px-2 py-0.5 text-xs rounded bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400"><%= @type["kind"] %></span>
        <h4 class="text-sm font-semibold text-gray-900 dark:text-gray-100"><%= @type["name"] %></h4>
      </div>
      <p :if={@type["description"]} class="mt-1 text-xs text-gray-500 dark:text-gray-400"><%= @type["description"] %></p>
      <%= if @type["fields"] && @type["fields"] != [] do %>
        <div class="mt-2 border-t border-gray-100 dark:border-gray-700 pt-2 space-y-1">
          <%= for field <- @type["fields"] do %>
            <div class="flex items-center gap-2 text-xs">
              <span class="font-mono text-gray-700 dark:text-gray-300"><%= field["name"] %></span>
              <span class="font-mono text-indigo-500 dark:text-indigo-400"><%= type_display(field["type"]) %></span>
            </div>
          <% end %>
        </div>
      <% end %>
      <%= if @type["enumValues"] && @type["enumValues"] != [] do %>
        <div class="mt-2 border-t border-gray-100 dark:border-gray-700 pt-2">
          <span class="text-xs text-gray-500 dark:text-gray-400">Values:</span>
          <div class="flex flex-wrap gap-1 mt-1">
            <%= for val <- @type["enumValues"] do %>
              <span class="px-2 py-0.5 text-xs font-mono bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-300 rounded"><%= val["name"] %></span>
            <% end %>
          </div>
        </div>
      <% end %>
