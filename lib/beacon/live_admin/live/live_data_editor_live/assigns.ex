defmodule Beacon.LiveAdmin.LiveDataEditorLive.Assigns do
  ...
  def handle_params(params, _url, socket) do
    ...
    socket =
      socket
      |> assign(live_data: Content.live_data_for_path(socket.assigns.beacon_page.site, path))
  ...
  def handle_event("live_data_assign_editor_lost_focus", %{"value" => value}, socket) do
    ...
    changeset =
      ...
      |> Content.change_live_data(selected, %{
        "value" => value,
        "assign" => form.params["key"] || Map.fetch!(form.data, :key),
    ...
    socket =
      case Content.update_live_data_assign(site, selected, attrs) do
        {:ok, live_data_assign} ->
          ...
          socket
          |> assign(live_data: Content.live_data_for_path(site, live_data_path))
          |> assign_form()
          |> assign(unsaved_changes: false)
          |> push_patch(to: path)
        ...
        <div class="grid items-start grid-cols-1 grid-rows-1 mx-auto gap-x-8 gap-y-8 lg:mx-0 lg:max-w-none lg:grid-cols-3">
          ...
          <div :if={@form} class="w-full col-span-2">
            <.form :let={f} for={@form} class="flex items-end gap-4" phx-change="validate" phx-submit="save_changes">
              ...
              <input type="hidden" name="live_data_assign[value]" id="live_data_assign-form_value" value={@changed_value} />
  ...
  defp assign_selected(socket, key) do
    ...
    assign(socket,
      selected: selected,
      # changed_template: selected.value,
      changed_code: selected.value
    )
  end
